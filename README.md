...
- `constructor(options?)`
  - options:
    ...
    - bucketThreshold: Above this number of bytes, a record will be stored in a separate but common file, called bucket (simple Buffers are saved as binaries) (default: 2000).
    ...
    - encoder: A function passed to JSON.stringify for custom serialization (optional). Receives `(key, value, skip)` — return `skip` to leave the value as-is.
    - digArrays: Whether the encoder should recurse into array entries (default: false).
    - dontWriteSameValue: Skip writing when the new value is identical to the one on disk (default: true).
    - fileCollisionSeparator: Separator appended to deduplicate file names generated by fileThreshold (default: `'~'`).
    - cleanupOnOpen: Remove stale temporary files left by interrupted rewrites when opening (default: true).
...
- `open(path: string, { clear }?): Promise<void>`
